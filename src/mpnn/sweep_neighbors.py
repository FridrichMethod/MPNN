...
def get_neighbors_sweep(args):
    ...
    print(f"Current Epochs: {current_epochs}, Higher Epochs: {higher_epochs}, Lower Epochs: {lower_epochs}")
    print(f"Current Weighr Decay: {current_wd}, Higher WD: {higher_wd}, Lower WD: {lower_wd}")
    return higher_lr_args, lower_lr_args, higher_epochs_args, lower_epochs_args, higher_wd_args, lower_wd_args
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--partition", type=int, default=0, help="0: args, 1: higher_lr, 2: lower_lr, 3: higher_epochs, 4: lower_epochs, 5: higher_wd, 6: lower_ wd")
    args = argparser.parse_args()
    higher_lr_args, lower_lr_args, higher_epochs_args, lower_epochs_args, higher_wd_args, lower_wd_args  = get_neighbors_sweep(args)
    if args.partition == 0:
        train(args)
