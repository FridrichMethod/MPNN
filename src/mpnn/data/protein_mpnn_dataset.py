...
Adapted from https://github.com/pyg-team/pytorch_geometric/blob/master/torch_geometric/datasets/protein_mpnn_dataset.py
"""
...
class ProteinMPNNDataset(InMemoryDataset):
    ...
    raw_url = {
        "small": "https://files.ipd.uw.edu/pub/training_sets/pdb_2021aug02_sample.tar.gz",
        "large": "https://files.ipd.uw.edu/pub/training_sets/pdb_2021aug02.tar.gz",
    }
    ...
    def process(self) -> None:
        ...
        with tqdm(total=total_items, desc="Processing") as pbar:
            for _, items in cluster_ids.items():
                for chain_id, _ in items:
                    ...
                    if len(my_dict["seq"]) > self.max_length:
                        pbar.update(1)
                        continue
                    bad_chars = set(list(my_dict["seq"])).difference(alphabet_set)
                    if len(bad_chars) > 0:
                        pbar.update(1)
                        continue
            ...
            # The original code logic in PyG seems to just iterate through one cluster_ids.items()
            # But here we are iterating through ALL cluster items from _process_split(), which returns
            # only the items for the current split?
            # No, _process_split returns the data for self.split.
    ...
    def _process_split(self) -> dict[int, list[tuple[str, int]]]:
        ...
        if os.path.exists(save_path):
            ...
            df = df[(df["RESOLUTION"] <= self.rescut) & (df["DEPOSITION"] <= self.datacut)]
            ...
            for _, r in tqdm(df.iterrows(), desc="Processing split", total=len(df)):
    ...
    def _process_pdb1(self, chain_id: str) -> dict[str, Any]:
        ...
        asmb_candidates = {a for a, b in zip(asmb_ids, asmb_chains) if chid in b.split(",")}
        ...
        homo = {ch_j for seqid_j, ch_j in zip(seqid, chids) if seqid_j > self.homo}
