...
class StaBddG(nn.Module):
    ...
    def __init__(self, pmpnn, use_antithetic_variates=True, noise_level=0.1, l_to_r=False, device='cuda'):
    ...
    def folding_ddG(self, domain, mut_seqs, set_wt_seq=None):
        ...
        ddG = mut_dG - wt_dG
        ddG *= -1 # use the convention that a negative value (ddG < 0) represents a stabilizing mutation.
        return ddG
    ...
    def folding_dG_batched(self, domain_list, seqs_list, decoding_order=None, backbone_noise=None):
        ...
        """
        B = len(domain_list)
        X_, _, mask_, _, chain_M_, residue_idx_, _, chain_encoding_all_ = featurize(domain_list, self.device)
        ...
        X_, mask_, chain_M_ = X_.repeat_interleave(num_mut_seqs, dim=0), mask_.repeat_interleave(num_mut_seqs, dim=0), chain_M_.repeat_interleave(num_mut_seqs, dim=0)
        residue_idx_, chain_encoding_all_ = residue_idx_.repeat_interleave(num_mut_seqs, dim=0), chain_encoding_all_.repeat_interleave(num_mut_seqs, dim=0)
        ...
        if self.use_antithetic_variates:
            ...
            backbone_noise = backbone_noise * S_mask[..., None, None] # this line is to make sure that padded positions have no noise

        log_probs = self.pmpnn(X_, S_, mask_, chain_M_, residue_idx_, chain_encoding_all_, fix_order=decoding_order, fix_backbone_noise=backbone_noise)
    ...
    def folding_ddG_batched(self, domain_list, mut_seqs_list):
        ...
        wt_dG = self.folding_dG_batched(domain_list, wt_seq, decoding_order=decoding_order, backbone_noise=backbone_noise)
        mut_dG = self.folding_dG_batched(domain_list, mut_seqs_list, decoding_order=decoding_order, backbone_noise=backbone_noise)
        ...
        ddG_batched *= -1 # use the convention that a negative value (ddG < 0) represents a stabilizing mutation.
        return ddG_batched
